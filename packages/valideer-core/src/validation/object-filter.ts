...
export const parseObjectFilter = <T>(
  ...
  if (typeof value === "string" || Array.isArray(value)) {
    switch (subType) {
      ...
      case "string":
        if (options?.enum) {
          return { [`$${filter}`]: parseEnumList(value, options.enum) };
        }
    ...
    for (const filter of filters) {
      ...
      if (valueList.length > 0) {
        ...
        switch (subType) {
          ...
          case "string":
            if (options?.enum) {
              filterObject[filterKey] = parseEnumList(valueList, options.enum);
            } else {
...
export const IsObjectFilter = () => {
  return (object: Record<string, any>, propertyName: string): void => {
    registerDecorator({
      ...
      constraints: [],
      options: {
        message: `${propertyName} is not a valid object filter`,
      },
      validator: {
        validate: (value: any) => {
          console.log(value);
          ...
          //       (x) => typeof x === "string" || isStringArray(x),
          //     );
          //   };
          //
          ...
          //     const parsed = parseObjectFilter(value, subType);
          //
